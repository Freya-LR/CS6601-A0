...
Please download [Miniconda (Python3.x)](https://docs.conda.io/en/latest/miniconda.html) and install it on your local machine. Note that it does not matter which version of Miniconda you download, as this will only affect the base environment. You will be creating a separate environment in the next step, for which you can specify the version of Python required in the environment creation command itself. You can access conda via the console to make sure it's properly installed. For instance, you can run `conda -V` to display the version.
